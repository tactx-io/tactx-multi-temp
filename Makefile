# Main makefile
# inspired by https://www.mikrocontroller.net/articles/Beispiel_Makefile
# slightly modified and stripped down to the necessary parts
#----------------------------------------------------------------------------
...
OBJ = $(CSRC:%.c=./%.o) $(CPPSRC:%.cpp=./%.o) $(ASRC:%.S=./%.S.o)

# Define all listing files.
LST = $(CSRC:%.c=./%.lst) $(CPPSRC:%.cpp=./%.lst) $(ASRC:%.S=./%.lst)
...
# Default target.
all: begin dump checksource gccversion sizebefore build sizeafter end
...
dump:
	@echo "> BUILD REPORT" > source.txt
	@echo "project:    $(PROJECT)"  >> source.txt
	...
	@echo "not used:" >> source.txt
	@$(foreach src, $(CPPSRC-n), echo $(src)>> source.txt;)
	@echo "> C Source Files" >> source.txt
	@$(foreach src, $(CSRC), echo $(src)>> source.txt;)
	@echo "" >> source.txt
	@echo "> Assembler Source Files" >> source.txt
	@$(foreach src, $(ASRC), echo $(src)>> source.txt;)
	@echo "" >> source.txt
	@echo "> C++ Source Files" >> source.txt
	@$(foreach src, $(CPPSRC), echo $(src)>> source.txt;)
	@echo "" >> source.txt
	@echo "Object Files" >> source.txt
	@$(foreach src, $(OBJ), echo $(src)>> source.txt;)
	@echo "" >> source.txt
	@echo "Include Dirs Files" >> source.txt
	@$(foreach src, $(INC), echo $(src)>> source.txt;)
	@echo "" >> source.txt
	@echo "Configs" >> source.txt
	@echo $(CONFIG_LUFA)>> source.txt;
...
clean_list :
	...
	$(REMOVE) $(TARGET).sym
	$(REMOVE) $(TARGET).lss
	$(REMOVE) $(CSRC:%.c=./%.o) $(CPPSRC:%.cpp=./%.o) $(ASRC:%.S=./%.o)
	$(REMOVE) $(CSRC:%.c=./%.lst) $(CPPSRC:%.cpp=./%.lst) $(ASRC:%.S=./%.lst)
	$(REMOVE) $(CSRC:.c=.d)
	$(REMOVE) $(CSRC:.c=.i)
	$(REMOVEDIR) .dep

checksource:
	@for f in $(CSRC) $(CPPSRC) $(ASRC); do \
		if [ -f $$f ]; then \
